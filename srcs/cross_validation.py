...
wandb.init(project="cross_validation", entity="gg1224")
config = wandb.config
...
def train(model, DEVICE, train_loader, optimizer, epoch):
    ...
    for inputs, targets in train_loader:
        ...
        loss.backward()
        optimizer.step()
    loss_sum = loss_sum/(math.ceil((1633*0.8)/config.batch_size))
    if (epoch + 1) % 100 == 0:
...
for train_id, test_id in kfold.split(dataset):
    model.apply(init_weights)
    train_dataset = Subset(dataset, train_id)
    test_dataset = Subset(dataset, test_id)
